...
export async function settingsCmd(ctx: any) {
  ...
  const lines = [
    ...
    "",
    "Risk & sizing",
    `Risk per trade: ${s.riskPerTradeUSD > 0 ? `$${s.riskPerTradeUSD}` : "not set (feeds off; copy uses copysize)"}`,
    `Max positions: ${s.maxPositions}`,
    `Copy size ratio: ${s.copySizeRatio > 0 ? `${s.copySizeRatio}x (spotware copy signals open at ${s.copySizeRatio}x the source size)` : `${off} (copy signals are risk-sized)`}`,
